...
module.exports = function (deployer) {
  ...
  let model = fs.readFileSync(
    "./src/ml-models/hot_dog-not/classifier-perceptron-400.json",
    "utf8"
  );
  ...
  deployer.then(async () => {
    ...
    const incentiveMechanism = await deployer.deploy(
      Stakeable64,
      refundTimeWaitTimeS,
      ownerClaimWaitTimeS,
    ...
    console.log(
      `  Deployed incentive mechanism to ${incentiveMechanism.address}.`
    );

    console.log(
      ` Deploying classifier with first ${Math.min(
        weights.length,
        weightChunkSize
      )} weights.`
    );
    const classifier = await deployer.deploy(
      DensePerceptron,
      classifications,
      weights.slice(0, weightChunkSize),
      intercept,
      learningRate,
    ...
    for (let i = weightChunkSize; i < weights.length; i += weightChunkSize) {
      console.log(
        ` Deploying classifier weights [${i}, ${Math.min(
          i + weightChunkSize,
          weights.length
        )}).`
      );
    ...
    if (featureIndices !== undefined) {
      if (featureIndices.length !== weights.length) {
        throw new Error(
          "The number of features must match the number of weights."
        );
      }
      for (let i = 0; i < featureIndices.length; i += weightChunkSize) {
        console.log(
          ` Deploying classifier feature indices [${i}, ${Math.min(
            i + weightChunkSize,
            featureIndices.length
          )}).`
        );
        await classifier.addFeatureIndices(
          featureIndices.slice(i, i + weightChunkSize)
        );
      }
    }
    ...
    console.log(
      `  Deployed Hot Dog collaborative classifier to ${instance.address}.`
    );
    ...
    return axios
      .post(`${pjson.proxy}api/models`, modelInfo)
      .then(() => {
      ...
      .catch((err) => {
        if (
          process.env.CI !== "true" &&
          process.env.REACT_APP_ENABLE_SERVICE_DATA_STORE === "true"
        ) {
